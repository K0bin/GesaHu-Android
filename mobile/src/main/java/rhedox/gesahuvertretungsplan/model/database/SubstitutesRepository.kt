...
class SubstitutesRepository(context: Context) : android.support.v4.content.Loader.OnLoadCompleteListener<Cursor> {
    ...
    fun loadSubstitutesForDay(date: LocalDate) {
        ...
        if(loader == null) {
            loader = CursorLoader(context.applicationContext, SubstitutesContract.uriWithDate(date), SubstitutesContract.columns.toTypedArray(), null, null, "${SubstitutesContract.columnLessonBegin} ASC");
