...
class SubstitutesRepository(context: Context) : android.support.v4.content.Loader.OnLoadCompleteListener<Cursor> {
    ...
    init {
        observer = Observer {
            if(it.pathSegments.size > 1 && it.pathSegments[1] == "date") {
                if(it.pathSegments[0] == SubstitutesContentProvider.substitutesPath)
                    loadSubstitutesForDay(localDateFromUnix(it.lastPathSegment.toInt()));
                else if(it.pathSegments[0]== SubstitutesContentProvider.announcementsPath)
    ...
    fun loadSubstitutesForDay(date: LocalDate) {
        ...
        if(loader == null) {
            loader = CursorLoader(context.applicationContext, Uri.parse("content://${SubstitutesContentProvider.authority}/${SubstitutesContentProvider.substitutesPath}/date/${date.unixTimeStamp}"), Substitutes.availableColumns.toTypedArray(), null, null, "${Substitutes.columnLessonBegin} ASC");
    ...
    fun loadAnnouncementForDay(date: LocalDate) {
        ...
        if(loader == null) {
            loader = CursorLoader(context.applicationContext, Uri.parse("content://${SubstitutesContentProvider.authority}/${SubstitutesContentProvider.announcementsPath}/date/${date.unixTimeStamp}"), Announcements.availableColumns.toTypedArray(), null, null, null);
            loader.registerListener(id, this)
